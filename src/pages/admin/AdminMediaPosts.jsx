...
function PostModal({ item, mediaProjectId, onClose, onSave }) {
  ...
  const handleSave = async () => {
    ...
    try {
      if (item?.id) {
        await api.admin.updateNewsPost(item.id, { ...form, resident_id: null });
      } else {
        await api.admin.createNewsPost({ ...form, resident_id: null });
      }
      onSave();
    } finally {
      setSaving(false);
...
export default function AdminMediaPosts() {
  ...
  const load = async () => {
    setLoading(true);
    const [projects, postsData] = await Promise.all([
      api.admin.listMediaProjects(),
      api.admin.listNews(),
    ]);
    setProject(projects.find((item) => item.id === id) || null);
    setPosts(postsData.filter((item) => item.media_project_id === id));
    setLoading(false);
  };
  ...
  return (
    <div className="pt-24 min-h-screen px-6 py-12 max-w-screen-xl mx-auto">
      <Link to="/admin/media" className="flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-white/30 hover:text-white transition-colors mb-8 group">
        <ArrowLeft size={14} className="group-hover:-translate-x-1 transition-transform" />
        Назад к медиапроектам
      </Link>

      <div className="flex items-center justify-between mb-8">
        <div>
          <p className="text-xs font-bold uppercase tracking-widest text-cyber-violet mb-1">Публикации медиапроекта</p>
          <h1 className="text-3xl font-black uppercase text-white">{project?.title || '...'}</h1>
        </div>
        <button
              ...
              <div className="flex items-center gap-4 min-w-0">
                ...
                <div className="min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    {post.video_url && <span className="text-xs text-blue-400 font-bold">▶ Видео</span>}
