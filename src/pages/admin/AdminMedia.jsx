...
export default function AdminMedia() {
  ...
  const load = async () => {
    setLoading(true);
    const data = await api.admin.listMediaProjects();
    setItems(data);
    setLoading(false);
  };
  ...
  const handleDelete = async (id) => {
    ...
    await api.admin.deleteMediaProject(id);
    load();
  };
  ...
  return (
    <div className="pt-24 min-h-screen px-6 py-12 max-w-screen-xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-8">
        <h1 className="text-3xl font-black uppercase text-white">Медиапроекты</h1>
        <button
      ...
      {loading ? (
        ...
        <p className="text-white/40">Нет проектов. Добавьте первый!</p>
      ) : (
          ...
          {items.map((item) => (
            <div key={item.id} className="border border-white/10 p-5 flex items-center justify-between gap-4">
                  ...
                  <div className="flex items-center gap-2">
                    {item.social_url && (
                      <span className="text-xs text-green-400 flex items-center gap-1">
                        <ExternalLink size={10} /> Соцсеть
                      </span>
                    )}
                    {!item.social_url && item.body && (
                      <span className="text-xs text-white/30">Внутренняя страница</span>
                    )}
              ...
              <div className="flex items-center gap-2 shrink-0">
                <Link to={`/admin/media/${item.id}/posts`} className="p-2 text-white/40 hover:text-cyber-violet transition-colors" title="Публикации">
                  <FileText size={16} />
                </Link>
